	...
	if ($dwtby_gender_value == 2) {
		if (($daowat_to == $user) || ($user_gender_value == 2)) {
			...
			<form action='daowat_cmntsFrame.php?id=".$getid."' method='POST' name='daowatComment".$getid."'>
				<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; border: 1px solid #0B810B;' name='daowat_body' placeholder= 'Leave your comment here!' autofocus>
		...
		$get_msg_num = mysql_query("SELECT * FROM pvt_messages WHERE user_from='$daowat_to' AND user_to='$user' LIMIT 2");
			...
			if ($female_msg >=1 ) {
				echo "
					<div>
					<form action='daowat_cmntsFrame.php?id=".$getid."' method='POST' name='daowatComment".$getid."'>
						<input style='padding: 10px 3px; min-width: 60px; width: 73%; margin: 0 0 5px 0; resize: none; border: 1px solid #0B810B;' name='daowat_body' placeholder= ''>
						<input type='submit' name='daowatComment".$getid."' class='commentSubmit' value='Comment'>
					</form>
					</div>
				";
